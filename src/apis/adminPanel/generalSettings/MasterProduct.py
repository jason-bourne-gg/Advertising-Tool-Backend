...
masterproductlist_blueprint = Blueprint('masterproductlist', __name__)

masterproductlist_store = {}

@masterproductlist_blueprint.route('/uploadmasterproductlist', methods=['POST'])
@login_required
def uploadmasterProductList():
    ...
    custom_file_name = f'{directory_name.lower()}' +'_default_product_list'
    ...
    try:
        ...
        blob_name = f'ProductTrackingLists/DEFAULT/{directory_name}/' + custom_file_name + ".csv"
        ...
        return jsonify({"message": "ASIN file uploaded successfully!"}), 200
...
@masterproductlist_blueprint.route('/downloadmasterproductlist', methods=['POST'])
@login_required
def downloadMasterProductTrackingList():
    try:
